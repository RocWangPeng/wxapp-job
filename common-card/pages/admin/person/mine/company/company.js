...
Page({
	...
	cardByInfoType(cardId) {
		...
		utils.request(utils.personApi + '/personal/get/cardByInfoType', 'GET', data)
			.then(res => {
				if (res.code == 0) {
					var tempShowArr = []
					var cardCompanyProfilePhotosUrl = res.data.info.cardCompanyProfilePhotosUrl
					var cardCompanyProfilePhotosUrlOld = res.data.info.cardCompanyProfilePhotosUrl
					...
					this.setData({
						introduce: res.data.info.cardCompanyProfile == 'null' ? ' ' : res.data.info.cardCompanyProfile,
						tempFilePaths: tempShowArr,
						cardCompanyProfilePhotosUrl:cardCompanyProfilePhotosUrlOld
					})
					if(res.data.info.cardCompanyProfileDisplayStatus){
						this.setData({
							isShow: res.data.info.cardCompanyProfileDisplayStatus,
						})
					}
				}
			})
	...
	formSubmit(e) {
		...
		var data = {
			id: this.data.cardId,
			cardCompanyProfile: this.data.introduce,
			cardCompanyProfileDisplayStatus: this.data.isShow,
		...
		utils.request(utils.personApi + '/personal/update/card/company/profile', 'POST', data,
		...
		uploadimg(data) {
			...
			wx.uploadFile({
				url: utils.personApi + '/personal/update/card/company/profile',
				header: {
					'cookie': "_auth=" + wx.getStorageSync('_auth'),
					"Content-Type": "multipart/form-data"
				},
				filePath: data.path[i],
				name: 'companyProfileImage', //这里根据自己的实际情况改
			...
			wx.showModal({
			  ...
			  success(res) {
				if (res.confirm) {
					...
					utils.request(utils.personApi + '/personal/delete/card/companyProfileImg', 'POST', data,'application/x-www-form-urlencoded')
						.then(res=>{
