...
// 	},
	...
	submitMsg() {
		...
		const activeUnionId = wx.getStorageSync('userUnionId')

		var data = {
			msgType:'2',
			sourceName:that.data.userName,
			sourcePhone:that.data.tel,
			msgContent:that.data.question,
			cardId:activeCardId,
			unionId:activeUnionId
		}
		utils.request(utils.personApi + '/personal/send/advice', 'POST', data)
			.then(res=>{
				if(res.code == 0){
					wx.showModal({
						title: '提示',
