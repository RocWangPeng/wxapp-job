...
Page({
			...
			.then((res) => {
				if (res.code == 0) {
					...
					if (self.data.cardInfo.cardShowPhotosUrls) {
						var showImgs = []
						var NewshowImgs = []
						  showImgs = self.data.cardInfo.cardShowPhotosUrls.split('h_1000,')
						  for(var i =0;i<showImgs.length-1;i++){
							  NewshowImgs.push(showImgs[i]+'h_1000')
						  }
						
						self.data.cardInfo.cardShowPhotosUrls = NewshowImgs
					} else {
	...
	onShareAppMessage: function() {
		return {
			...
			path: '/pages/card/team/show/show?cardId=' + this.data.cardInfo.id,
